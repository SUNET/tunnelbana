...
An empty or malformed `disco_endpoints` list is a build-time config error.
...
| Attacker-chosen `entityID` steering an open flow | The value is routing input, not authorization: backend selection is gated by `custom_routing`'s mandatory `(issuer, requester)` allowlist, and the selected backend still resolves the entity through signature-verified MDQ / federation metadata before sending anything | A forged return riding the SameSite=None cookie while a flow is open can pre-select a different *federation-trusted* IdP - the same exposure class as a forged `?idphint` (ADR 0016), surfaced to the user at the IdP login page |
