...
### PQC signing keys for OIDC

Standard builds enable jose-rs 0.7.0's `post-quantum` support for OIDC and OpenID
Federation signing and verification. Supported JOSE algorithm names are:

- `ML-DSA-44`, `ML-DSA-65`, `ML-DSA-87`
- `ML-DSA-44-ES256`, `ML-DSA-65-ES256`, `ML-DSA-87-ES384`
- `ML-DSA-44-Ed25519`, `ML-DSA-65-Ed25519`, `ML-DSA-87-Ed448`

Use an AKP JWK generated by jose-rs, supplied through `signing_jwk` or
`signing_jwk_path`, and set `signing_algorithm` explicitly to its `alg` value.
The PEM loader and automatic algorithm inference do not support these keys.
For example, in an `oidc` frontend:

```toml
signing_jwk_path  = "keys/op-mldsa65.jwk"
signing_algorithm = "ML-DSA-65"
signing_key_id    = "op-pqc-1"
```

For an `oidc` or `oidc_federation` backend verifying this OP's tokens, set:

```toml
id_token_signed_response_alg = "ML-DSA-65"
```

This verification policy is independent of `signing_algorithm`, which controls
outbound client assertions; the federation key has its own `signing_algorithm`
under `[backend.config.federation]` or `[frontend.config.federation]`. Federation
request objects and entity configurations can also use these keys. Each peer
must support the selected algorithm and jose-rs's AKP representation. Composite
